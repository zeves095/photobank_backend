...
## Use cases
 ...
 - Поиск ресурсов.
 - Скачивание ресурсов.
   - Массовое скачивание ресурсов.
 - Загрузка ресурсов для товаров на сервер.
 - Изменение метаинформации ресурсов, их типа и приоритета.
 - Работа с хранилищем, отдельным от каталога.
   - Добавление разделов.
   - Удаление разделов.
   - Изменение имени разделов.
...
**Ресурс** - Изображение, которое хранится в примонтированной директории, имеет запись в базе данных и привязано к товару каталога, либо к ноде свалки
**Тип ресурса** - Изображение может быть основным(1), дополнительным(2) или исходником(3). Для основного изображения генерируются все пресеты. Исходники - любые загруженные файлы, выставляется по умолчанию. Пресеты(4) - четвертый тип ресурса. Существует только в связке с одним из трех типов и не отображается как самостоятельный в основном интерфейсе.
**Пресет** - Автоматически сгенерированные изображения определенного размера. Thumbnail(1) - 320/180, Medium(2) - 1920/1080, Large(3) - 3840/2160. Все имеют тип 4 в базе. Привязаны к ресурсам через gid.
**Приоритет ресурса** - Очередность для дополнительных ресурсов (тип 2).
**Свалка** - Второй "каталог", в котором вместо привязки к товарам каталога, ресурсы расположены в хаотичном порядке

---

## Сущности

Перед ознакомлением советую посмотреть [словарь терминов](#markdown-header-словарь-терминов)
 ...
 - **GarbageNode** - раздел "свалки", то есть раздел хранилища, не относящийся к каталогу товаров.
    *deleted* - влияет только на отображение. "Удаленные" ноды никуда не деваются и их можно восстановить
 ...
 - **Link** - Ссылка на ресурс для внешнего доступа. Может указывать на существующий ресурс либо на сгенерированныый файл с нестандартным размером.
    *path* - Путь до физического ресурса, на который будет смотреть ссылка
    *access* - Ограничение по ip адресам, на которые можно отдавать картинку
    *target* - Имя группы ссылок, для огранизции
    *expires_by* - Время, после которого ссылка будет неактивна
    *max_requests* - Максимальное количество запросов на эту ссылку
    *done_requests* - Сколько раз ссылка была запрошена
    *ready* - Готова ли ссылка, или он все еще ждет обновления/генерации картинки
    *symlink* - Смотрит ли ссылка на ресурс каталога или имеетсобственный файл.
 - **Upload** - Относится к *FileUploaderBundle*. Является начатой, но не законченной загрузкой одного файла.
    *total_chunks* - Общее ожидаемое количество чанков для данного изображения
    *completed_chunks* - Количество успешно переданных чанков
    *file_hash* - md5-хэш файла, для идентификации в случа прерванной загрузки
...
На backend разница между ресурсом свалки и каталога определяется по первому символу кода 1с, 9 в случае свалки.

![alt text](./img/fileuploader-sequence.png)

### Свалка

В самом начале разработки было озвучено требование: файлы будут привязаны ТОЛЬКО к товарам каталога. Прямо перед запуском появилась совершенно непредсказуемая необходимость в отдельном хранилище. Так появилась на свет ***свалка***. Код свалки был вкручен в готовую систему, которая на свалку не расчитывала. Сущность GarbageNode - это просто копия CatalogueNode. Ресурсы, привязанные к нодам свалки, имеют пустое поле item и привязаны по garbageNode. Ноды свалки пронумерованы похожим образом на обычные ноды, но их id всегда начинается с цифры 9. Также ноды свалки можно создавать, удалять, перемещать и переименовывать. Все это происходит в Controller/GarbageStorageController. Для корректной работы в базе должна быть хотя бы одна нода с parent=null.

Требования к поведению удаленных разделов каталога:
 - Можно удалять разделы с дочерними разделами и ресурсами
 - При удалении раздела все его дочерние ресурсы также становятся удаленными
 - Не удаленные ресурсы не могут быть дочерними от удаленных
 - Соответственно, нельзя переместить не удаленный раздел в удаленный
 - Можно переместить удаленный раздел в любой другой
 - Удаленные разделы не отображаются в поиске
...
![alt text](./img/photobank-main-upload-sequence.png)
...
Для запусков end-to-end тестов через selenium нужно запустить в корневой директории *yarn e2e*.
Файлы тестов в директории */e2e/tests/*. В */e2e/utils* описаны общие методы для инициализации webdriver и сбора данных для тестов.
Полезные методы:
***sw(driver),w(driver),lw(driver)*** - Короткое, среднее и долгое ожидание
***waitForEl(driver, selector)*** - Ждет, пока на странице не появится элемент по css селектору
***getVerifiedData(driver)*** - Получает данные из каталога
...
Ряд операций для работы с системой реализованы через консоль Symfony. Классы определены в отдельном бандле и подключаются через composer [(packagist)](https://packagist.org/packages/photobank/dev-console-commands-bundle)
